...
:  
argc(ac), argv(av), print_license(FALSE), help(FALSE), checking(TRUE), prob_rules(FALSE),
no_compression(TRUE), hash_compression(FALSE), hash_disk(FALSE), bpctl_prob_ord(FALSE), 
...
void for_prob_ord()
{
  ...
  // sprintf(filetmp, "/tmp/%s.tmp", args->filename);
  // codefile = fopen(filetmp[1], "r");
  // if (codefile != NULL) {
  //   fprintf(stderr,
  ...
  // fclose(codefile);
  sprintf(buffer,
	  "grep numrules %s.cpp | awk '{print $5}' | awk -F';' '{printf(\"#define NUMRULES_DEFINE %%d\\n\", $1)}' > %s.cpp.tmp; cat %s.cpp >> %s.cpp.tmp; mv %s.cpp.tmp %s.cpp",
	  args->filename, args->filename, args->filename, args->filename, args->filename, args->filename);
